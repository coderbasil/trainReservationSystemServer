...
export async function BookSeat(trainId, occupantIds, seatClass, totalPrice) {
  const connection = await pool.getConnection();

  try {
    await connection.beginTransaction();

    // 1) Fetch enough FREE seats for occupantIds.length
    const [seatRows] = await connection.query(
      `
        ...
        LIMIT ?
      `,
      [trainId, seatClass, occupantIds.length]
    );

    if (seatRows.length < occupantIds.length) {
      // Not enough free seats to accommodate all
      await connection.rollback();
      return { payed: false, message: "Not enough free seats." };
    }

    // 2) Identify the "main passenger" (assume occupantIds[0])
    //    so we can check their balance and deduct cost
    const mainPassengerId = occupantIds[0];
    if (!mainPassengerId.startsWith("P")) {
      // In case occupantIds[0] is actually a Dependent
      // you may need your own logic to find which occupant is the passenger
      await connection.rollback();
      return { payed: false, message: "No main passenger in occupantIds[0]." };
    }

    // 3) Lock that passenger's row via SELECT ... FOR UPDATE
    const [[passengerRow]] = await connection.query(
      `
        SELECT passenger_id, balance
        FROM Passengers
        WHERE passenger_id = ?
        FOR UPDATE
      `,
    ...
    if (!passengerRow) {
      // Passenger not found
      await connection.rollback();
      return { payed: false, message: "Passenger not found in DB." };
    }

    if (passengerRow.balance < totalPrice) {
      // Insufficient balance
      await connection.rollback();
      return { payed: false, message: "Insufficient balance." };
    }

    // 4) All good: proceed with seat booking
    //    We'll mark all reservations as "Confirmed" since we are fully paying
    ...
    for (let i = 0; i < occupantIds.length; i++) {
      ...
      const seatId = seatRows[i].seat_id;

      let passengerId = null;
      let dependentId = null;

      // Distinguish occupant type by prefix
      if (occupantId.startsWith("D")) {
        dependentId = occupantId;
      } else {
        passengerId = occupantId;
      }

      // Insert into Reservations
      const [reservationRes] = await connection.query(
        `
          INSERT INTO Reservations 
            (passenger_id, dependent_id, train_id, status, reservation_date)
          VALUES (?, ?, ?, ?, NOW())
        `,
        [passengerId, dependentId, trainId, status]
      );
      const reservationId = reservationRes.insertId;
      ...
      await connection.query(
        ...
        `,
        [ticketId, seatId, trainId]
      );
    }

    // 5) Deduct the totalPrice from the passenger's balance
    const newBalance = passengerRow.balance - totalPrice;
    await connection.query(
      `
        UPDATE Passengers
        SET balance = ?
        WHERE passenger_id = ?
      `,
      [newBalance, mainPassengerId]
    );

    // 6) Update train seat availability
    await connection.query(
      `
        UPDATE Trains T
        SET available_cabin_seats = (
          SELECT total_cabin_seats - COUNT(*)
          FROM Seats S
          WHERE S.train_id = T.train_id 
            AND S.seat_status = 'Booked' 
            AND S.seat_class = 'Cabin'
        )
        WHERE T.train_id = ?
      `,
      [trainId]
    ...
    await connection.query(
      `
        UPDATE Trains T
        SET available_firstclass_seats = (
          SELECT total_firstclass_seats - COUNT(*)
          FROM Seats S
          WHERE S.train_id = T.train_id 
            AND S.seat_status = 'Booked'
            AND S.seat_class = 'First Class'
        )
        WHERE T.train_id = ?
      `,
      [trainId]
...
// export async function BookSeat(trainId, ids, seatClass, price) {
//   let userId = user;
